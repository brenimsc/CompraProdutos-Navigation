...
private const val COMPRA_PAGAMENTO = "Compra realizada"
...
class PagamentoFragment : Fragment() {
        ...
        if (::produtoEscolhido.isInitialized) {
            viewModel.salva(pagamento)
                .observe(this, Observer {
                    it?.dado?.let{
                        Toast.makeText(context, COMPRA_PAGAMENTO, Toast.LENGTH_SHORT).show()
                        val direcoes =
    ...
    private fun geraPagamento(numeroCartao: String, dataValidade: String, cvc: String): Pagamento? = try {
        Pagamento(
