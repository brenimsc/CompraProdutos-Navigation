...
class DetalhesProdutoFragment : Fragment() {
    ...
    private fun configuraBotaoComprar() {
        detalhes_produto_botao_comprar.setOnClickListener {
            viewModel.produtoEncontrado.value?.let{produtoSelecionado ->
                val direcoes = DetalhesProdutoFragmentDirections.actionDetalhesProdutosToPagamento(produtoSelecionado.id)
